...
export default function HireExperts({ setCurrentPage, onOpenConsultation }: HireExpertsProps) {
  ...
  return (
    <div className="relative overflow-hidden py-16 sm:py-20 bg-slate-50 text-slate-800" id="hire-experts-page">
      {/* Background ambient gradient loops */}
      <div className="absolute left-[-15%] top-10 h-[600px] w-[600px] rounded-full bg-blue-500/5 glow-blur pointer-events-none" />
      <div className="absolute right-[-15%] bottom-10 h-[600px] w-[600px] rounded-full bg-indigo-500/5 glow-blur pointer-events-none" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 space-y-16">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 max-w-6xl mx-auto" id="experts-showcase-grid">
          {filteredExperts.map((exp, idx) => (
            <motion.div
              ...
              className="flex flex-col gap-6 p-6 rounded-2xl border border-slate-250/70 bg-white shadow-sm hover:shadow-md hover:border-blue-500/20 transition-all duration-300 relative"
            >
              ...
              <div className="flex-grow space-y-4">
                <div>
                  ...
                  <p className="text-xs font-semibold text-blue-600 mt-0.5">
                    {exp.title} &bull; <span className="text-slate-500 font-normal">{exp.experience} Exp</span> &bull; <span className="text-slate-500 font-normal">{exp.location}</span>
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {exp.certifications.map((cert, i) => (
                      <span 
                        key={i} 
                        className="inline-block px-2 py-0.5 rounded bg-slate-100 border border-slate-200/50 text-slate-700 text-3xs font-medium leading-relaxed"
                      >
                        {cert}
                      </span>
                    ))}
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-100">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-xs text-slate-650">
          ...
          {filteredExperts.length === 0 && (
            <div className="col-span-full py-12 text-center border border-dashed border-slate-300 rounded-2xl bg-white">
              <RefreshCw className="h-8 w-8 text-slate-400 mx-auto animate-spin mb-3" />
              <p className="text-sm font-semibold text-slate-600">No experts currently available matching {selectedTech}.</p>
              <button 
        ...
        <div className="grid hidden grid-cols-1 lg:grid-cols-12 gap-8 max-w-6xl mx-auto rounded-3xl border border-slate-200 bg-white shadow-lg overflow-hidden" id="interactive-squad-calculator">
          
          {/* Left panel: sliders and configuration */}
          <div className="lg:col-span-7 p-6 sm:p-10 space-y-8 bg-slate-50/50 border-r border-slate-100">
            <div className="space-y-2">
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded bg-indigo-50 border border-indigo-150 text-indigo-600 text-3xs font-mono font-bold uppercase tracking-widest">
                Resource Modeling
              </span>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono bg-blue-50 text-blue-600 px-2 py-0.5 rounded font-bold">{dataEngineers} Headcount</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                ...
                />
                <p className="text-4xs text-slate-400">Expert pipelines: Databricks, Snowflake, Apache Kafka, Apache Spark, and dbt layouts.</p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded font-bold">{aiArchitects} Headcount</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                ...
                />
                <p className="text-4xs text-slate-400">AI native features: Sovereign RAG infrastructure, LLM tuning, LangChain, and Guardrail engines.</p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded font-bold">{biAnalysts} Headcount</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                ...
                />
                <p className="text-4xs text-slate-400">Business intelligence: Looker dashboards, PowerBI workspaces, dbt Semantic layer definitions.</p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono bg-rose-50 text-rose-600 px-2 py-0.5 rounded font-bold">{qaEngineers} Headcount</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                  ...
                  className="w-full accent-rose-600 bg-slate-200 h-1.5 rounded-lg appearance-none cursor-pointer"
                />
                <p className="text-4xs text-slate-400">Quality verification: CI/CD test automations, Playwright frameworks, data validation integrity checkpoints.</p>
              </div>

              {/* Duration Slider */}
              <div className="space-y-2 pt-4 border-t border-slate-200/55">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-800">
                    Project Duration Commitment
                  </span>
                  <span className="font-mono bg-slate-100 text-slate-700 px-2.5 py-0.5 rounded font-bold">{squadDuration} Months</span>
                </div>
                <input 
                  type="range" 
                  min="1" 
                  ...
                  className="w-full accent-slate-600 bg-slate-200 h-1.5 rounded-lg appearance-none cursor-pointer"
                />
                <p className="text-4xs text-slate-400">Minimum B2B project tenure is 1 month. Longer tenures qualify for certified SLA architectures.</p>
              </div>
            </div>
          </div>

          {/* Right panel: calculated output & booking inquiry */}
          <div className="lg:col-span-5 p-6 sm:p-10 flex flex-col justify-between space-y-8 bg-slate-100/40">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-3">
                  <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-3xs">
                    ...
                    <div className="text-xl font-extrabold text-slate-900 font-display mt-0.5">{totalSquadSize} FTEs</div>
                  </div>
                  <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-3xs">
                    <div className="text-3xs text-slate-400 font-medium">Estimated Weekly Core Hours</div>
                    <div className="text-xl font-extrabold text-slate-900 font-display mt-0.5">{totalSquadSize * 40} Hrs</div>
                  </div>
                </div>
              </div>
            ...
            <div className="pt-6 border-t border-slate-200">
              {inquirySent ? (
                <div className="bg-emerald-500/5 border border-emerald-500/20 text-emerald-600 rounded-xl p-4 text-xs space-y-2 text-center">
                ...
                <form onSubmit={handleInquirySubmit} className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input 
                      ...
                      className="rounded bg-white border border-slate-200 px-3 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500"
                    />
                    <input 
                      type="text" 
                      required
        ...
        <div className="rounded-2xl border border-slate-250 bg-white shadow-sm p-8 max-w-4xl mx-auto space-y-6" id="compliance-assurance-banner">
          ...
          <p className="text-xs text-slate-650 text-center max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-3xs font-mono text-slate-550 uppercase tracking-widest pt-2">
